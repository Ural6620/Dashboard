    ...
    <table class="min-w-full divide-y divide-gray-300">
      <thead>
        <tr>
          ...
          <th scope="col" class="th">Maktab nomi</th>
          <th scope="col" class="th">Aloqa</th>
      ...
      <tbody class="bg-white">
        ...
        >
          ...
          <td class="td">
            <div>
              {{ item.translates?.at(0)?.address || '' }}
            </div>
            <div>
              {{ item.translates?.at(0)?.phone || '' }}
            </div>
          </td>
          <td class="td">
...
const { schools } = storeToRefs(store)

const getData = async () => {
  await store.getschools({
    limit: limit.value
  })
}
...
const remove = async (answer) => {
  if (answer) {
    await store.removeschool(_id.value)
  }
  close()
}
