  ...
  <default-modal>
    ...
    >
      ...
      <DialogTitle as="h3" class="text-lg font-semibold leading-6 text-gray-900">
        {{ edit ? 'Maktabni tahrirlash' : 'Yangi maktab' }}
      </DialogTitle>

      <div class="space-y-2 mt-4">
        <default-input
          v-model="data.translate.title"
          name="title"
          label="Maktab nomi"
      ...
      <div class="space-y-2 mt-4">
        <default-input
          v-model="data.translate.address"
          name="address"
          label="Manzil"
          :error="v$.translate.address.$invalid && v$.translate.address.$dirty"
        />
      </div>
      <div class="space-y-2 mt-4">
        <phone-maska
          v-model="data.translate.phone"
          name="phone"
      ...
      <div class="space-y-2 mt-4">
        <default-select
          v-model="data.translate.language"
          name="language"
          label="Tilni tanlang"
...
import { schoolStore } from '@/stores/data/school'
const store = schoolStore()

const send = async () => {
  v$.value.$touch()
  if (!v$.value.$invalid) {
    if (edit.value) await store.saveschool({ ...data.value })
    else await store.addschool({ ...data.value })
...
watch(
  ...
  async () => {
    if (id?.value?.length > 0 && lang?.value?.length > 0) {
      const res = await store.getschool(id.value, lang.value)
